...
public class AppComponent {
    ...
    private static boolean flagflag = true;
    private static int activePortnum = 0;
    private static int passivePortnum = 0;
    private static boolean active  = false;
    private static boolean passive = false;

    private final Logger log = getLogger(getClass());

    private static HashMap<HashMap<Integer, Integer>, Boolean> statefw
			= new HashMap<HashMap<Integer, Integer>, Boolean>();
    ...
    @Property(name = "ipv6Forwarding", boolValue = true,//false,
            label = "Enable IPv6 forwarding; default is false")
    private boolean ipv6Forwarding = true;//false;
    ...
    @Property(name = "matchIpv4Address", boolValue = true,//false,
            label = "Enable matching IPv4 Addresses; default is false")
    private boolean matchIpv4Address = true;//false;
    ...
    @Property(name = "matchIpv6Address", boolValue = true,//false,
            label = "Enable matching IPv6 Addresses; default is false")
    private boolean matchIpv6Address = true;//false;
    ...
    @Property(name = "matchTcpUdpPorts", boolValue = true,//false,
            label = "Enable matching TCP/UDP ports; default is false")
    private boolean matchTcpUdpPorts = true;//false;

    @Property(name = "matchIcmpFields", boolValue = true,//false,
            label = "Enable matching ICMPv4 and ICMPv6 fields; " +
                    "default is false")
    private boolean matchIcmpFields = true;//false;
    ...
    public void activate(ComponentContext context) {
        ...
        appId = coreService.registerApplication("org.P2_Firewall.app");
        ...
        public void process(PacketContext context) {
            ...
            if (matchIpv4Address && inPkt.getEtherType() == Ethernet.TYPE_IPV4) {
                ...
                if (matchTcpUdpPorts && ipv4Protocol == IPv4.PROTOCOL_TCP) {
                    ...
                    String tcp_payload = new String(tcpPacket.getPayload().serialize());
                    log.info("tcp payload : {}, src.port : {}, dst.port : {}" ,tcp_payload,tcpPacket.getSourcePort(),tcpPacket.getDestinationPort());

                    if (tcp_payload.contains("PORT")){

                        String[] tokens = tcp_payload.split(",");
                        //log.info("lenght : {}",tokens[5].length());
                        //tokens[5].substring(0,tokens[5].length()-3);
/*
                        for (int i = 0; i<tokens.length;i++){
                            if (i == 5) {
                                log.info("WOW!!!! : {}", tokens[i].substring(0,tokens[5].length()-2));
                            }
                        }
*/


                        activePortnum = Integer.parseInt(tokens[4])*256 + Integer.parseInt(tokens[5].substring(0,tokens[5].length()-2));
                        log.info ("port number : {}",activePortnum);
                        active = true;


                    }

                    if (tcp_payload.contains("PASV")){
                    ...
                    if (passive){
                        if (tcp_payload.contains("passive")){
                        ...
                        for (int i = 0; i<tokens.length;i++){
                            ...
                            //activePortnum = Integer.parseInt(tokens[4])*256 + Integer.parseInt(tokens[5].substring(0,tokens[5].length()-2));
                            //log.info ("port number : {}",passivePortnum);
    ...
    private void installRule(PacketContext context, PortNumber portNumber) {
        ...
        if (matchDstMacOnly) {
            ...
            if (matchIpv4Address && inPkt.getEtherType() == Ethernet.TYPE_IPV4) {
                ...
                IPv4 ipv4Packet = (IPv4) inPkt.getPayload();
                byte ipv4Protocol = ipv4Packet.getProtocol();
    	        HashMap<Integer, Integer> connectionmap
	                        			= new HashMap<Integer, Integer>();
		        connectionmap.put (ipv4Packet.getSourceAddress(), ipv4Packet.getDestinationAddress());

		        if (!statefw.containsKey (connectionmap)){
			        statefw.put (connectionmap, false);
			        //log.info ("IP connection Added");
		        }
                //log.info ("IP_DestinationAddress : {}", ipv4Packet.getDestinationAddress());
                ...
                if (matchTcpUdpPorts && ipv4Protocol == IPv4.PROTOCOL_TCP) {
                    ...
                    String tcp_payload = new String (tcpPacket.getPayload().serialize());
                    //log.info("tcp payload : " + tcp_payload);

                    if (statefw.get(connectionmap) != true) {
                        if (tcpPacket.getDestinationPort() == 21) {
                            /*
                            flagflag = true;
                            */

                            // ClientSide
                            statefw.remove(connectionmap);
                            statefw.put(connectionmap, true);

                            // ServerSide
                            HashMap<Integer, Integer> connectionmap2
                                    = new HashMap<Integer, Integer>();
                            connectionmap2.put (ipv4Packet.getDestinationAddress(), ipv4Packet.getSourceAddress());

                            if (!statefw.containsKey (connectionmap2)){
                                statefw.put (connectionmap2, true);
                                log.info ("IP connection Added");
                            }
                            else{
                                statefw.remove(connectionmap2);
                                statefw.put(connectionmap2,true);
                            }
                            flagflag = false;
                ...
                if (matchIcmpFields && ipv4Protocol == IPv4.PROTOCOL_ICMP) {
                    ...
                    //log.info("type {} code {}",icmpPacket.getIcmpType(),icmpPacket.getIcmpCode());
                    selectorBuilder.matchIPProtocol(ipv4Protocol)
        ...
        if (active){ //&& tcpPacket.getDestinationPort() == activePortnum){

            //if (tcpPacket.getSourcePort() != activePortnum && tcpPacket.getDestinationPort() != activePortnum) {
            if (tcpPacket.getDestinationPort() == 21 || tcpPacket.getSourcePort() == 21) {
            ...
            else if (tcpPacket.getDestinationPort() == activePortnum || tcpPacket.getSourcePort() == activePortnum) {
                TrafficTreatment treatment = DefaultTrafficTreatment.builder()
                        .setOutput(portNumber)
                        .build();
                ForwardingObjective forwardingObjective = DefaultForwardingObjective.builder()
            ...
            if (tcpPacket.getDestinationPort() == 21 || tcpPacket.getSourcePort() == 21) {
                ...
                /*
                    flowObjectiveService.forward(context.inPacket().receivedFrom().deviceId(),
                            forwardingObjective);
                            */
                log.info("paypaypay : {}",tcp_payload);
            ...
            if (tcp_payload.contains("passive")){
                ...
                */
                ...
                passivePortnum = Integer.parseInt(tokens[4])*256 + Integer.parseInt(tokens[5].substring(0,tokens[5].length()-4));
                log.info ("port number : {}",passivePortnum);
            ...
            if (passive){
                ...
                else if (tcpPacket.getDestinationPort() == passivePortnum || tcpPacket.getSourcePort() == passivePortnum) {
