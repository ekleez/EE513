...
public class AppComponent {
    ...
    private static int one_to_three = 0;
    private static int three_to_one = 0;
    ...
    private static int redirected = 0;
    ...
    public void activate(){//ComponentContext context) {
        ...
        appId = coreService.registerApplication("org.P2_Redirection.app");
    ...
    private void installRule(PacketContext context, PortNumber portNumber) {
        ...
        log.info("Port : {} Dst Mac : {} ",portNumber.toString(),inPkt.getDestinationMAC());
        ...
        */
        ...
        IpAddress ip_format;
        MacAddress mac_format;

        IPv4 ipv4Packet_pp = (IPv4) inPkt.getPayload();

        ip_format = IpAddress.valueOf(IPv4.fromIPv4Address(ipv4Packet_pp.getDestinationAddress()));

        mac_format = inPkt.getDestinationMAC();
        ...
        if (matchDstMacOnly) {
            ...
            if (matchIpv4Address && inPkt.getEtherType() == Ethernet.TYPE_IPV4) {
                ...
                //log.info("ip????{}",ipv4Packet.getSourceAddress());

                // Incoming Packet

                //log.info("dest : {}, src: {}",ipv4Packet.getDestinationAddress(),ipv4Packet.getSourceAddress());

                if(ipv4Packet.getDestinationAddress()%10 == 2){

                    if (ipv4Packet.getSourceAddress()%10 == 1){

                        // Add
                        //ipv4Packet.setDestinationAddress(ipv4Packet.getDestinationAddress() + 1);


                        //IpAddress ip_format = new IpAddress(Version.INET, ipv4Packet.serialize());
                        ip_format = IpAddress.valueOf(IPv4.fromIPv4Address(ipv4Packet.getDestinationAddress() + 1));

                        mac_format = MacAddress.valueOf(3);

                        /*
                        treatment = DefaultTrafficTreatment.builder()
                                .setIpDst(ip_format)
                                .setEthDst(mac_format)
                                .setOutput(portNumber)
                                .build();
                        */

                        one_to_three = one_to_three + 1;

                    }
                    else if (ipv4Packet.getSourceAddress()%10 == 3){
                        ...
                        ip_format = IpAddress.valueOf(IPv4.fromIPv4Address(ipv4Packet.getDestinationAddress() - 1));
                        mac_format = MacAddress.valueOf(1);
                ...
                // Outgoing Packet
                    ...
                    if (one_to_three != 0){
                        if (ipv4Packet.getSourceAddress()%10 == 3){

                            //ipv4Packet.setSourceAddress(ipv4Packet.getSourceAddress() - 1);

                            //IpAddress ip_format = new IpAddress(Version.INET, ipv4Packet.serialize());

                            ip_format = IpAddress.valueOf(IPv4.fromIPv4Address(ipv4Packet.getSourceAddress() - 1));
                            mac_format = MacAddress.valueOf(2);
                ...
                if (matchIcmpFields && ipv4Protocol == IPv4.PROTOCOL_ICMP) {
                    ...
                    //log.info("from {} to {}",ipv4Packet.getSourceAddress(),ipv4Packet.getDestinationAddress());
