...
public class AppComponent {
    ...
    public void activate(ComponentContext context) {
        ...
        Thread play = new Thread (new Playing_thread());
        play.start();
    }
    ...
    Map<FlowId, Long> value_list = new HashMap<>();
    ...
    public void analyze() {
        ...
        for (TopologyVertex v: graph.getVertexes()) {
            ...
            for (FlowEntry flow: flowTable) {
                ...
                if (!value_list.containsKey (flow.id())){

                    value_list.put(flow.id(), flow.bytes());

                }else{

                    long old_value = value_list.get (flow.id());
                    long new_value = flow.bytes();

                    value_list.put (flow.id(), flow.bytes());

                    if (new_value - old_value > 10485760){

                        MacAddress find_mac = ((EthCriterion)(flow.selector().getCriterion(Criterion.Type.ETH_SRC))).mac();
                        Iterator<Host> ip_host = (hostservice.getHostsByMac(find_mac)).iterator();

                        log.info("Byte : {}",new_value - old_value);
                        //log.info("??? : {}", flow.selector().getCriterion(Criterion.Type.ETH_SRC));
                        log.info("FlowRule : {}",flowEntryRepresentation(flow));
                        while(ip_host.hasNext()) {
                            log.info("Sender_IP : {}", ip_host.next().ipAddresses().toString());
                        }
    ...
    class Playing_thread extends Thread {
